...
def load_boardlife_reviews(path="../../01_data/boardgame/boardlife_reviews_final.csv"):
    """
    보드라이프 리뷰 데이터 로드.
    임베딩 텍스트 : review_text
    메타데이터    : rank, title, rating, source (고정값: "boardlife")
    """
    df = pd.read_csv(path)
    ...
    # 메타데이터 컬럼 목록 (source는 아래에서 직접 주입)
    meta_cols = ["rank", "title", "rating"]
